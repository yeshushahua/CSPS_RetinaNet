...
class CSPDarknet(nn.Module):
    ...
    def forward(self, x):
        feat1, feat2, feat3 = self.model(x)
        return [feat1, feat2, feat3]
...
class retinanet(nn.Module):
    def __init__(self, num_classes, phi, pretrained=False, fp16=False):
        ...
        #   C4     38,38,1024
        #   C5     19,19,2048
        #   当我们使用CSPDarknet53的时候
        #   C3     75,75,256
        #   C4     38,38,512
        #   C5     19,19,1024
        #-----------------------------------------#
        if phi == 5:
            self.backbone_net = CSPDarknet(pretrained)
            fpn_sizes = [256, 512, 1024]
            # 添加SimAM注意力模块
            self.simam_C3 = SimAM()
        else:
    ...
    def forward(self, inputs):
        ...
        #   C5     19,19,2048
        #-----------------------------------------#
        [C3, C4, C5] = self.backbone_net(inputs)

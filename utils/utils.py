...
def download_weights(backbone, model_dir="./model_data"):
    ...
    # 检查本地是否已有权重文件
    if backbone == '5':
        local_weight_path = os.path.join(model_dir, 'CSPdarknet53_backbone_weights.pth')
        if os.path.exists(local_weight_path):
            print(f'Found local weights at {local_weight_path}')
            return
    ...
    if backbone not in download_urls:
        print(f'No download URL for backbone {backbone}, skipping download')
        return
